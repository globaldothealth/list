...
_COMORBIDITIES_MAP = {
    ...
    "doenças renais crônicas": "chronic kidney disease",
...
def parse_cases(raw_data_file: str, source_id: str, source_url: str):
    ...
    with open(raw_data_file, "r") as f:
        ...
        for row in reader:
            ...
            if row[_TEST_RESULT] == "Positivo" and row[_OUTCOME] != "Cancelado" and row[_STATE] == "CEARÁ" and confirmation_date is not None:
                try:
                    case = {
                        "caseReference": {"sourceId": source_id, "sourceEntryId": row[_UUID], "sourceUrl": source_url},
                        "location": {
                            "query": ", ".join(
                                [row[_MUNICIPALITY], "Ceará", "Brazil"]
                            )
                        },
