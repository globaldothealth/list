...
# Load manual geocoding dictionaries, made from Colombia municipality -> coordinate lookup table
...
mun_code_coord = dictionaries["mun_code_coord"]
mun_code_place_name = dictionaries["mun_code_place_name"]
...
#                         "verificationStatus": "VERIFIED",
                ...
                if entry["Fecha de diagnóstico"]:
                    ...
                    notes.append("No Date of Diagnosis provided, so using Date Reported Online (fecha reporte web) as a proxy. This is normally approx. 1 week later.")

                # If patient was symptomatic, mark date of onsetSymptoms, otherwise
                # asymptomatic
                if entry["Fecha de inicio de síntomas"]:  # maybe change to elif clause and check if can parse field as date
                    case["symptoms"] = {
                        "status": "Symptomatic",
                    }
                    case["events"].append({
                ...
                if entry["Estado"].lower() in symptom_map.keys():
                    case["symptoms"]["values"] = [symptom_map.get(entry['Estado'].lower())]
                ...
                # Travel History - we currently do not have any travel dates, so unknown whether in last 30 days 
                if entry["Tipo de contagio"].lower() == "importado":
                    notes.append(f"Case is reported as importing the disease into Colombia, and country of origin is {entry['Nombre del país']}.")
                    ...
                    notes.append("Case transmission under investigation (currently unknown).")

                if entry['fecha reporte web']:
                    notes.append(f"Date reported online was {convert_date(entry['fecha reporte web'],dataserver=False)}.")
                if entry['Fecha de notificación']:
                    notes.append(f"Date reported to SIVIGILA was {convert_date(entry['Fecha de notificación'],dataserver=False)}.")

                if entry['Tipo de recuperación'] == 'PCR':
                    notes.append(f"Patient recovery was confirmed by a negative PCR test.")
                elif entry['Tipo de recuperación'] == 'Tiempo':
                    notes.append(f"Patient recovery was confirmed by 21 days elapsing with no symptoms.")
