...
const validEvents = ['firstClinicalConsultation', 'onsetSymptoms', 'selfIsolation', 'confirmed', 'hospitalAdmission', 'icuAdmission', 'outcome'];
const dateOnlyEvents = ['firstClinicalConsultation', 'onsetSymptoms', 'selfIsolation'];
...
export const denormalizeFields = (doc: CaseDocument): Partial<CaseDocument> => {
    const caseReferenceFields = denormalizeCaseReferenceFields(doc.caseReference);
    ...
    const preexistingConditionsFields = denormalizePreexistingConditionsFields(doc.preexistingConditions);
    const revisionMetadataFields = denormalizeRevisionMetadataFields(doc.revisionMetadata);
    ...
    const travelHistoryFields = denormalizeTravelHistoryFields(doc.travelHistory);
    const vaccineHistory = denormalizeVaccineFields(doc.vaccines);
    const variantFields = denormalizeVariantFields(doc.variant);

    const nestedFields = ['caseReference', 'demographics', 'events', 'location', 'pathogens', 'preexistingConditions',
        'revisionMetadata', 'symptoms', 'transmission', 'travelHistory', 'vaccines', 'variant'];

    const undesiredFields = ['list', 'importedCase'];

    const flatFields = [caseReferenceFields, demographicsFields, eventFields, locationFields, pathogenFields,
        preexistingConditionsFields, revisionMetadataFields, symptomsFields, transmissionFields, travelHistoryFields,
        vaccineHistory, variantFields];
...
function denormalizeCaseReferenceFields(doc: CaseReferenceDocument): Record<string, string> {

    let denormalizedData: Record<string, string> = {};
    let additionalSources: string[] = [];
    if (doc.hasOwnProperty('additionalSources')) {
        for (let source of doc.additionalSources || []) {
            additionalSources.push(source.sourceUrl);
        }
    }
    denormalizedData['caseReference.additionalSources'] = additionalSources.join(',');
    ...
    const uploadIds = (doc.uploadIds === undefined) ? '' : doc.uploadIds.join(',');
    denormalizedData['caseReference.uploadIds'] = '';
    denormalizedData['caseReference.uploadIds'] = uploadIds;
    denormalizedData['caseReference.verificationStatus'] = doc.verificationStatus || '';
    return denormalizedData;
}

function denormalizeDemographicsFields(doc: DemographicsDocument): Record<string, string|number> {

    let denormalizedData: Record<string, string|number> = {};
    ...
    const nationalities = (doc.nationalities === undefined) ? '' : doc.nationalities.join(',');
    denormalizedData['demographics.nationalities'] = nationalities;
    denormalizedData['demographics.occupation'] = doc.occupation || '';
    return denormalizedData;
}

export const denormalizeEventsFields = (docs: [EventDocument]): Record<string, string> => {
    let denormalizedData: Record<string, string> = {};

    for (let event of docs) {
        if (validEvents.indexOf(event.name) !== -1) {
            const end = (event.dateRange?.end === undefined) ? '' : event.dateRange.end.toString();
            denormalizedData[`events.${event.name}.date`] = end;
            if (dateOnlyEvents.indexOf(event.name) === -1) {
                denormalizedData[`events.${event.name}.value`] = event.value || '';
            }
        }
    }
    for (let event of validEvents) {
        ...
        if (!(field in denormalizedData) && dateOnlyEvents.indexOf(field) === -1) {
            denormalizedData[field] = '';
        }
    }
    return denormalizedData;
...
function denormalizeLocationFields(doc: LocationDocument): Record<string, string|number> {
    let denormalizedData: Record<string, string|number> = {};

    denormalizedData['location.administrativeAreaLevel1'] = doc.administrativeAreaLevel1 || '';
    denormalizedData['location.administrativeAreaLevel2'] = doc.administrativeAreaLevel1 || '';
    denormalizedData['location.administrativeAreaLevel3'] = doc.administrativeAreaLevel1 || '';
    denormalizedData['location.country'] = doc.country || '';
    denormalizedData['location.geoResolution'] = doc.geoResolution || '';
    denormalizedData['location.geometry.latitude'] = doc.geometry?.latitude || '';
    denormalizedData['location.geometry.longitude'] = doc.geometry?.longitude || '';
...
function denormalizePathogenFields(docs: [PathogenDocument]): Record<string, string> {
    let denormalizedData: Record<string, string> = {};
    let pathogens = [];
    if (docs !== undefined) {
        for (let doc of docs) {
...
function denormalizePreexistingConditionsFields(doc: PreexistingConditionsDocument): Record<string, string|boolean> {
    let denormalizedData: Record<string, string|boolean> = {};

    denormalizedData['preexistingConditions.hasPreexistingConditions'] = doc?.hasPreexistingConditions || '';
    if (doc === undefined) {
...
function denormalizeRevisionMetadataFields(doc: RevisionMetadataDocument): Record<string, string|number> {
    let denormalizedData: Record<string, string|number> = {};

    denormalizedData['revisionMetadata.creationMetadata.curator'] = doc.creationMetadata?.curator || '';
    denormalizedData['revisionMetadata.creationMetadata.date'] = doc.creationMetadata?.date?.toString() || '';
    denormalizedData['revisionMetadata.creationMetadata.notes'] = doc.creationMetadata?.notes || '';
    denormalizedData['revisionMetadata.editMetadata.curator'] = doc.updateMetadata?.curator || '';
    denormalizedData['revisionMetadata.editMetadata.date'] = doc.updateMetadata?.date.toString() || '';
    denormalizedData['revisionMetadata.editMetadata.notes'] = doc.updateMetadata?.notes || '';
    denormalizedData['revisionMetadata.revisionNumber'] = doc.revisionNumber || '';
    return denormalizedData;
}

function denormalizeSymptomsFields(doc: SymptomsDocument): Record<string, string> {
    let denormalizedData: Record<string, string> = {};
...
function denormalizeTransmissionFields(doc: TransmissionDocument): Record<string, string> {
    let denormalizedData: Record<string, string> = {};
    ...
    if (doc === undefined) {
        ...
        const linkedCaseIds = (doc.linkedCaseIds === undefined) ? '' : doc.linkedCaseIds.join(',');
...
function denormalizeTravelHistoryFields(doc: TravelHistoryDocument): Record<string, string|boolean> {
    let denormalizedData: Record<string, string|boolean> = {};
    ...
    if (doc !== undefined && doc.hasOwnProperty('travel')) {
        for (let travelDoc of doc.travel) {
            ...
            const travelMethods = (travelDoc.methods === undefined) ? '' : travelDoc.methods.join(',');
    ...
    denormalizedData['travelHistory.travel.location.country'] = location_countries.join(',');
    denormalizedData['travelHistory.travel.location.name'] = location_names.join(',');
    denormalizedData['travelHistory.travel.methods'] = methods.join(';');
    denormalizedData['travelHistory.travel.purpose'] = purposes.join(',');

    denormalizedData['travelHistory.traveledPrior30Days'] = doc?.traveledPrior30Days || '';
    return denormalizedData;
}

function denormalizeVaccineFields(docs: [VaccineDocument]): Record<string, string> {
    let denormalizedData: Record<string, string> = {};
    ...
    for (let i = 0; i < 4; i++) {
        if (docs === undefined || docs[i] === undefined) {
            ...
            const sideEffects = (doc.sideEffects === undefined) ? '' : doc.sideEffects.values?.join(',') || '';
...
function denormalizeVariantFields(doc: VariantDocument): Record<string, string> {
    let denormalizedData: Record<string, string> = {};
