...
class Point(Document):
    ...
    def validate(self):
        ...
        if len(self.coordinates) != 2:
            raise ValidationError(f"Point must have two coordinates, I have {len(self.coordinates)}")
        latitude = self.coordinates[0]
        if latitude < -90.0 or latitude > 90.0:
            raise ValidationError(f"latitude must be between -90º and 90º, got {latitude}")
...
class Feature(Document):
    ...
    properties: Dict[str, Any] = dataclasses.field(init=False, default_factory = lambda: {})

    def validate(self):
        if self.type != "Feature":
            raise ValidationError(f"Type must be Feature for a Feature, not {self.type}")
        if not isinstance(self.geometry, Point):
            raise ValidationError(f"geometry of a Feature must be a Point in G.h, {self.geometry} is not one")
