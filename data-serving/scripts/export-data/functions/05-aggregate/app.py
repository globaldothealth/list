...
client = pymongo.MongoClient(f"mongodb+srv://{username}:{password}@covid19-map-cluster01.sc7u9.mongodb.net/covid19?retryWrites=true&w=majority")
db = client.covid19
cases = db.cases
...
def get_jhu_counts():
    ...
    jhu_counts = jhu_df['Confirmed'].groupby(jhu_df['Country_Region']).sum().reset_index()
    jhu_counts['Country_Region'] = jhu_counts['Country_Region'].apply(lambda x: re.sub(r'[^a-zA-Z ]', '', x))
    jhu_counts['Country_Region'] = jhu_counts['Country_Region'].apply(lambda x: _JHU_COUNTRY_MAP[x] if x in _JHU_COUNTRY_MAP.keys() else x)
...
def get_country_codes():
    ...
    countrycodes_filtered = countrycodes_df[['official_name_en', 'ISO3166-1-Alpha-2']].dropna().copy()
    countrycodes_filtered['official_name_en'] = countrycodes_filtered['official_name_en'].apply(lambda x: _CODES_COUNTRY_MAP[x] if x in _CODES_COUNTRY_MAP.keys() else x)
    countrycodes_series = pd.Series(data=countrycodes_filtered['ISO3166-1-Alpha-2'].values.flatten(), index=countrycodes_filtered['official_name_en'])
...
def generate_country_json():
    ...
    s3 = boto3.client('s3')
    s3.put_object(ACL='public-read', Body=json.dumps(records), Bucket='covid-19-aggregates', Key="country/latest.json")
    s3.put_object(ACL='public-read', Body=json.dumps(records), Bucket='covid-19-aggregates', Key=f"country/{now}.json")
...
def generate_region_json():
    ...
    s3 = boto3.client('s3')
    s3.put_object(ACL='public-read', Body=json.dumps(records), Bucket='covid-19-aggregates', Key="regional/latest.json")
    s3.put_object(ACL='public-read', Body=json.dumps(records), Bucket='covid-19-aggregates', Key=f"regional/{now}.json")
...
def generate_total_json():
    ...
    s3 = boto3.client('s3')
    s3.put_object(ACL='public-read', Body=json.dumps(record), Bucket='covid-19-aggregates', Key="total/latest.json")
    s3.put_object(ACL='public-read', Body=json.dumps(record), Bucket='covid-19-aggregates', Key=f"total/{now}.json")


def lambda_handler(event, context):
    country = generate_country_json()
    region = generate_region_json()
    generate_total_json()
