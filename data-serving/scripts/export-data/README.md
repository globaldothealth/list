...
```bash
data-export$ sam build --use-container
```
...
```bash
data-export$ sam logs -n StockCheckerFunction --stack-name data-export --tail
```
...
```bash
data-export$ pip install -r tests/requirements.txt --user
# unit test
data-export$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
data-export$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name data-export
```
